...
const UserTable = ({ 
  ...
  const handleSelectAll = (e) => {
    if (onSelectionChange) {
      onSelectionChange(e.target.checked ? users.map(u => u.id || u._id) : [])
    }
  }
  ...
  const handleSelectOne = (id) => {
    if (onSelectionChange) {
      const newSelection = selectedUserIds.includes(id)
        ? selectedUserIds.filter(sid => sid !== id)
        : [...selectedUserIds, id]
      ...
      <table className="min-w-full text-left text-sm">
          ...
          <tr>
            <th className="px-4 py-4 w-12">
              <input 
                ...
                className="rounded border-white/20 bg-transparent text-[#01F27B] focus:ring-[#01F27B] cursor-pointer h-4 w-4" 
              />
            </th>
        ...
        <tbody className="divide-y divide-white/10">
          {users.map((user) => {
            ...
            return (
              <tr key={userId} className={`text-white/80 transition-colors hover:bg-white/5 group ${isSelected ? 'bg-white/5' : ''}`}>
                <td className="px-4 py-4">
                  <input 
                    type="checkbox" 
                    checked={isSelected}
                    onChange={() => handleSelectOne(userId)}
                    className="cursor-pointer rounded border-white/20 bg-transparent text-[#01F27B] focus:ring-[#01F27B] h-4 w-4" 
                  />
                </td>
                <td className="px-4 py-4">
