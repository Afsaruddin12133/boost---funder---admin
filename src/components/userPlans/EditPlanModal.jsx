...
import { fieldClass, glassCardClass, outlineButtonClass, primaryButtonClass } from '../BoostFundrUI'
...
const EditPlanModal = ({ plan, onClose, onSaved }) => {
  ...
  return createPortal(
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      {/* Modal panel */}
      <div className={`relative z-10 w-full max-w-md overflow-hidden ${glassCardClass} shadow-2xl shadow-black/60`}>
        {/* Header */}
        <div className="flex items-center justify-between border-b border-white/10 px-6 py-4">
          ...
          <button
            onClick={onClose}
            className="flex h-8 w-8 items-center justify-center rounded-lg border border-white/10 bg-white/5 text-white/50 transition-all duration-300 hover:border-[#01F27B]/40 hover:text-white"
          >
        ...
        <div className="space-y-5 px-6 py-5">
          ...
          <div>
            ...
            <div className={`flex items-center gap-2 ${fieldClass} px-4 py-2.5`}>
          ...
          <div>
            ...
            <div className="min-h-[60px] flex flex-wrap gap-2 rounded-2xl border border-white/10 bg-white/5 p-3 transition-all duration-300 focus-within:border-[#01F27B]/40 focus-within:shadow-[0_0_40px_rgba(1,242,123,0.15)] backdrop-blur-xl">
              {features.map((f) => (
              ...
              <div className="flex items-center gap-1.5">
                ...
                <input
                  ...
                  className="min-w-[100px] bg-transparent text-xs text-white/70 outline-none placeholder:text-white/25"
                />
              </div>
            </div>
        ...
        <div className="flex items-center justify-end gap-3 border-t border-white/10 px-6 py-4">
          <button
            onClick={onClose}
            disabled={isSaving}
            className={`${outlineButtonClass} rounded-xl px-5 py-2.5 disabled:opacity-50`}
          >
            Cancel
          </button>
          <button
            onClick={handleSave}
            disabled={isSaving}
            className={`${primaryButtonClass} gap-2 rounded-xl px-5 py-2.5 disabled:opacity-60`}
          >
            {isSaving ? (
              <>
